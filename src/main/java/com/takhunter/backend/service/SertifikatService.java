...
public class SertifikatService {
    ...
    private Mahasiswa findValidMahasiswaForTerbit(Pendaftaran pendaftaran, Long mahasiswaId) {
        ...
        if (pendaftaran.getMahasiswaId() != null && pendaftaran.getMahasiswaId().equals(mahasiswa.getId())) {
            return mahasiswa;
        }
    ...
    private SertifikatMahasiswaResponse toMahasiswaResponse(Sertifikat sertifikat) {
        ...
        return SertifikatMahasiswaResponse.builder()
                ...
                .fileUrl(sertifikat.getDriveLink())
                .certificateUrl(sertifikat.getDriveLink())
                .poinTak(sertifikat.getPoinTak())
                .points(sertifikat.getPoinTak())
                .issuedAt(sertifikat.getIssuedAt())
                .tanggalTerbit(sertifikat.getIssuedAt())
                .status(sertifikat.getStatus().name())
                .build();
    }

    private Mahasiswa resolveMahasiswa(Pendaftaran pendaftaran) {
        if (pendaftaran.getMahasiswaId() != null) {
            Mahasiswa mahasiswa = mahasiswaRepository.findById(pendaftaran.getMahasiswaId()).orElse(null);
            if (mahasiswa != null) {
                return mahasiswa;
            }
        }
